...
namespace RevitSchedule
{
    public class ElectrSchedule
    {
        ...
        public ViewSchedule ElectrViewSchedule { get; set; }
        public ViewSchedule Host { get; set; }
        ...
        {
            using (Transaction ts1 = new Transaction(Doc, "sdf"))
            {
                ts1.Start();
        ...
        private ViewSchedule CreateSampleViewSchedule()
        {
            List<ViewSchedule> schedules = new FilteredElementCollector(Doc)
                               ...
                               .Cast<ViewSchedule>()
                               .ToList();
            using (Transaction ts2 = new Transaction(Doc, "sdf"))
            {
                ts2.Start();
        ...
        public void AddRowsWithElectrClass(List<ElectrClass> electrs)
        {
            using (Transaction ts1 = new Transaction(Doc, "sdf"))
            {
                ...
                for (int i = 0; i < electrs.Count; i++)
                {
                    ...
                    table.SetCellText(rowStart + i, 2, (el.P_all / 1000).ToString("F2"));
                    ...
                    table.SetCellText(rowStart + i, 6, (el.P_calc / 1000).ToString("F2"));
                    table.SetCellText(rowStart + i, 7, (el.Q_calc / 1000).ToString("F2"));
                    table.SetCellText(rowStart + i, 8, (el.S_calc / 1000).ToString("F2"));
                }
                ts1.Commit();
            }
        }
